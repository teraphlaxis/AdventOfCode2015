...
namespace Day6_Challenge2
{
	public partial class Form1 : Form
	{
		internal enum eInstruction
		{
			Toggle,
			TurnOn,
			TurnOff
		...
		internal class cInstruction
		{
			...
			public cInstruction(eInstruction a_Instruction, int a_StartX, int a_StartY, int a_EndX, int a_EndY)
			{
		...
		Bitmap m_Image;
		int[,] m_LightArray;
		List<cInstruction> m_Instructions;

		public Form1()
		{
			InitializeComponent();
			m_Image = new Bitmap(1000, 1000);
			m_LightArray = new int[1000, 1000];
			m_Instructions = new List<cInstruction>();
		...
		private void Form1_Load(object sender, EventArgs e)
		{
			...
			int t_LightBrightness = 0;
			int t_MaxBrightness = 0;
			
			for (int x = 0; x < 1000; x++)
			{
				for (int y = 0; y < 1000; y++)
				{
					t_LightBrightness += m_LightArray[x, y];
					if (m_LightArray[x, y] > t_MaxBrightness) { t_MaxBrightness = m_LightArray[x, y]; }
				}
			}
			...
			for (int x = 0; x < 1000; x++)
			{
				for (int y = 0; y < 1000; y++)
				{
					m_Image.SetPixel(x, y, Color.FromArgb((int)(((float)m_LightArray[x, y] / (float)t_MaxBrightness) * 255.0F), Color.Red));
				}
			}
			...
			Text += " - Light Brightness: " + t_LightBrightness;
		}

		private void ProcessInstructions()
		{
			foreach (cInstruction instruction in m_Instructions)
			{
				for (int x = instruction.StartX; x <= instruction.EndX; x++)
				{
					for (int y = instruction.StartY; y <= instruction.EndY; y++)
					{
						...
						{
							case eInstruction.Toggle:	m_LightArray[x, y] += 2;															break;
							case eInstruction.TurnOn:	m_LightArray[x, y] += 1;															break;
							case eInstruction.TurnOff:	m_LightArray[x, y] -= 1; if (m_LightArray[x, y] < 0) { m_LightArray[x, y] = 0; }	break;
						}
					}
				}
			}
		...
		{
			string t_Input = @"toggle 461,550 through 564,900
...
turn on 222,12 through 856,241";
			...
			// Process String to Instructions
			string[] t_Instructions = t_Input.Split('\n');
			
			// Clean Instructions of Return Character
			for	(int i = 0; i < t_Instructions.Length; i++)
			{
				if (t_Instructions[i].Contains("\r"))
				{
					t_Instructions[i] = t_Instructions[i].Substring(0, t_Instructions[i].Length - 1);
				}
			}

			// Create Instructions
			foreach (string instruction in t_Instructions)
			{
				string[] components = instruction.Split(' ');

				if (components.Length == 4)
				{
					string[] start = components[1].Split(',');
					string[] end = components[3].Split(',');

					m_Instructions.Add(new cInstruction(eInstruction.Toggle, int.Parse(start[0]), int.Parse(start[1]), int.Parse(end[0]), int.Parse(end[1])));
				}
				else
				{
					eInstruction operand = components[1] == "on" ? eInstruction.TurnOn : eInstruction.TurnOff;
					string[] start = components[2].Split(',');
					string[] end = components[4].Split(',');

					m_Instructions.Add(new cInstruction(operand, int.Parse(start[0]), int.Parse(start[1]), int.Parse(end[0]), int.Parse(end[1])));
				}
			}
		}
	}
