...
namespace Day3_Challenge2
{
	class Program
	{
		...
		{
			int t_S1_X = 0, t_S1_Y = 0;
			int t_S2_X = 0, t_S2_Y = 0;
			List<string> t_Houses = new List<string>();
			t_Houses.Add("0-0");

			for (int i = 0; i < m_Input.Length; i += 2)
			{
				// Santa
				switch (m_Input[i])
				{
				...
				string t_House = t_S1_X.ToString() + "-" + t_S1_Y.ToString();

				if (!t_Houses.Contains(t_House)) { t_Houses.Add(t_House); }
				...
				t_House = t_S2_X.ToString() + "-" + t_S2_Y.ToString();

				if (!t_Houses.Contains(t_House)) { t_Houses.Add(t_House); }
			}

			// Output and Halt
			Console.WriteLine("Houses that got Atleast one Present: " + t_Houses.Count);
			Console.ReadKey();
		}
	}
}
