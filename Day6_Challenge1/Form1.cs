...
namespace Day6_Challenge1
{
	public partial class Form1 : Form
	{
		...
		public Form1()
		{
			...
			m_LightArray = new bool[1000, 1000];
		...
		private void Form1_Load(object sender, EventArgs e)
		{
			...
			for (int x = 0; x < 1000; x++)
			{
				...
				{
					if (m_LightArray[x, y]) { t_LightsLit++; }
					m_Image.SetPixel(x, y, m_LightArray[x, y] ? Color.Red : Color.White);
				}
			}
			...
			foreach (cInstruction instruction in m_Instructions)
			{
				for (int x = instruction.StartX; x <= instruction.EndX; x++)
				{
					for (int y = instruction.StartY; y <= instruction.EndY; y++)
					{
						...
						{
							case eInstruction.Toggle:	m_LightArray[x, y] = !m_LightArray[x, y];	break;
							case eInstruction.TurnOn:	m_LightArray[x, y] = true;					break;
							case eInstruction.TurnOff:	m_LightArray[x, y] = false;					break;
						}
					}
				}
			}
